...
class PhotoPreviewTab(QWidget):
    ...
    def prev_image(self):
        ...
        # Validate image_paths exists and is not empty
        if not hasattr(self, 'image_paths') or not self.image_paths:
        ...
        if self.current_index > 0:
            ...
            logging.debug("PhotoPreviewTab: Already at the first image")
    ...
    def assign_lane(self, lane_code: str):
        ...
        if success:
            self.update_lane_display()
            self.timeline.repaint()  # Force immediate timeline repaint to show lane changes
            if hasattr(self.timeline, 'timeline_area'):
    ...
    def _enable_lane_change_mode(self, new_lane_code: str, timestamp: datetime):
        ...
        if success:
            ...
            self.update_lane_display()
            self.timeline.repaint()  # Force immediate timeline repaint to show auto-applied lane changes
            if hasattr(self.timeline, 'timeline_area'):
                self.timeline.timeline_area.repaint()
        else:
    ...
    def _exit_lane_change_mode(self):
        ...
        logging.info("PhotoPreviewTab: _exit_lane_change_mode called")
        ...
        if (hasattr(self, 'lane_change_start_timestamp') and self.lane_change_start_timestamp and
            ...
            if success:
                ...
                self.update_lane_display()
                self.timeline.repaint()  # Force immediate timeline repaint to show reverted lane colors
                if hasattr(self.timeline, 'timeline_area'):
                    self.timeline.timeline_area.repaint()
            else:
                logging.error("PhotoPreviewTab: Failed to revert lane change")
                self.timeline.repaint()  # Still update timeline in case of partial changes
                if hasattr(self.timeline, 'timeline_area'):
                    self.timeline.timeline_area.repaint()
        else:
            logging.warning("PhotoPreviewTab: Missing lane change state variables for revert")
    ...
    def clear_caches(self):
        ...
        if self.gps_data:
            self.gps_data = None
        self.events_per_fileid.clear()
        self.lane_fixes_per_fileid.clear()
    ...
    def save_lane_codes(self):
        ...
        if hasattr(self, 'current_fileid') and self.current_fileid and self.lane_manager:
            try:
                # Log current lane fixes before saving
                logging.info(f"PhotoPreviewTab: Saving {len(self.lane_manager.lane_fixes)} lane fixes:")
                for i, fix in enumerate(self.lane_manager.lane_fixes):
                    logging.info(f"  Fix {i}: {fix.from_time} to {fix.to_time}, lane='{fix.lane}', ignore={fix.ignore}")
                ...
                success = self.export_manager.export_lane_fixes(self.lane_manager.lane_fixes, output_path, include_file_id=False)
                if success:
